...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents + additional_contents) <= self.volume:
            ...
            vol_left = self.volume - len(self.contents)
            for k in range(vol_left):
                self.contents += additional_contents[k]
            for k in range(vol_left, len(additional_contents)):
                s = s + additional_contents[k]

        self.discarded += len(s)
        return s
    ...
    def shrink(self, new_volume):
        ...
        self.contents = ''
        s = self.append_string(temp_contents)
        return s
    ...
    def reset(self):
        ...
        self.history += [self.contents]  # also history.append(self.contents)
        print('history:', self.history, 'contents:', self.contents)
